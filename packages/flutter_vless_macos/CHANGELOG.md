...
* Hardened `prepare_apple_swiftpm.sh` for copied repository checkouts by precreating the stable `Flutter/ephemeral/Packages/.packages/flutter_vless_macos` link before Xcode resolves packages, normalizing generated SwiftPM paths, and clearing stale DerivedData package caches.
